...
#R_fire 
def r_fire(xy, tankx, tanky, turPos, r_firepower,xlocation,barrier_Width,randomheight, l_tankx, l_tanky):
    fire = True
    coordinates = list(xy)
    ...
    while fire:
        ...
        print(coordinates[0])
        coordinates[1] += int((((coordinates[0] - xy[0]) * 0.015/(r_firepower/50)) ** 2) - (turPos + turPos / (12 - turPos)))
        print(coordinates[1])
...
#L_fire
def l_fire(xy, tankx, tanky, turPos, l_firepower,xlocation,barrier_Width,randomheight,r_tankx, r_tanky):
    fire = True
    damage = 0
    coordinates = list(xy)
    clock = pygame.time.Clock()
    ...
    while fire:
        ...
        coordinates[0] += (12 - turPos) * 2
        print(coordinates[0])
        coordinates[1] += int((((coordinates[0] - xy[0]) * 0.015/(l_firepower/50)) ** 2) - (turPos + turPos / (12 - turPos)))
        print(coordinates[1])
        ...
        if coordinates[1] > display_height-ground_height:
            ...
            hit_x = int((coordinates[0] * display_height-ground_height) / coordinates[1])
            hit_y = int(display_height-ground_height)
            if r_tankx + 15 > hit_x > r_tankx - 15:
                damage = 25
...
def explosion(x, y, size=50):
    ...
    while explode:
        ...
        while magnitude < size:
            ...
            pygame.draw.circle(SCREEN, colorChoices[random.randrange(0, 4)], (exploding_x, exploding_y), random.randrange(1,5))
            magnitude += 1
            pygame.display.update()
            clock.tick(100)
        explode = False
...
def health_bars(player1,player2):
    if player1 > 75:
        ...
        get_Font(100).render("Gamer player-2 Wins")
    ...
    if player2 > 75:
        ...
        get_Font(100).render("Gamer player-1 Wins")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    r_fire(r_gun, r_tankx, r_tanky, currentTurPos, r_firePower,xlocation,barrier_Width,randomheight, l_tankx, l_tanky)
                    damage = l_fire(l_gun, l_tankx, l_tanky, l_currentTurPos, l_firePower,xlocation,barrier_Width,randomheight, r_tankx, r_tanky)                
                    player2 -= damage
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_a:
                    ...
                    l_fire(l_gun, l_tankx, l_tanky, l_currentTurPos, l_firePower,xlocation,barrier_Width,randomheight, r_tankx, r_tanky)
